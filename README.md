...
interface Manga {
  ...
  seriesCompleted: boolean // 완결 — 작품 연재가 끝났는가
  finishedReading: boolean // 완독 — 내가 끝까지 다 읽었는가
...
**완결**(작품 연재 종료)과 **완독**(내가 다 읽음)은 서로 다른 것이라 각각
독립된 체크 항목이다. 완결이지만 아직 안 읽은 작품, 연재 중인데 지금까지
나온 분량을 다 본 작품 모두 표현할 수 있다. 카드와 상세에 완결은 빨강,
완독은 검정 배지로 붙는다.

완독은 화수에서 추측하지 않고 체크한 값만 본다 — 전체 화수를 몰라도 완독
표시를 할 수 있어야 하기 때문이다. 대신 마지막 화까지 읽었는데 완독 체크가
안 돼 있으면 상세에 **완독으로 표시** 버튼이 뜬다. 체크는 상세 다이얼로그에서
바로 켜고 끌 수 있어서 수정 폼을 열 필요가 없다.
